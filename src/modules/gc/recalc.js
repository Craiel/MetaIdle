...
function recalc_gps(){
	...
	for (item in items){
		...
		if(item == "cursor"){
			...
			$("#{0}_gps".format(item)).html("Produces {0} GpS".format(digitgroup(items[item].gps * goomy.level * click_mult * global_mult, 1)));
		}else{
			gps += items[item].gps * items[item].count;
			$("#{0}_gps".format(item)).html("Produces {0} GpS".format(digitgroup(items[item].gps * global_mult, 1)));
		}
	}
